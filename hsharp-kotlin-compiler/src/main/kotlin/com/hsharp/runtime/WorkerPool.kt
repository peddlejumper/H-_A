...
class ConcurrentScope {
     ...
     *
     *  Parent-wins: as soon as one child fails, we cancel every
     *  still-pending sibling so we don't keep running doomed work. */
    fun join(): List<HValue> {
        ...
        for (c in children) {
            if (firstError != null) {
                // A prior child already failed: cancel this one if it
                // hasn't settled yet, then await to collect its result.
                c.cell.cancel()
            }
            try {
                ...
                if (firstError == null) {
                    firstError = t
                    // Cancel all remaining pending children so they
                    // don't outlive the failing sibling.
                    synchronized(childLock) {
                        for (other in children) {
                            if (!other.cell.isDone()) other.cell.cancel()
                        }
                    }
                }
            }
    ...
    /** Join variant used by CONCURRENT_EXIT: wait only for children
     *  that are still PENDING when this is called; skip children
     *  whose cell has already settled (RESOLVED / FAILED / CANCELLED).
     *
     *  Rationale: a settled cell was already observed by user code —
     *  typically via `await` inside a try/catch that swallowed the
     *  exception.  Re-awaiting a FAILED cell here would re-throw the
     *  original failure and defeat the user's try/catch, which was
     *  Bug 1 ("concurrent 块内 try/catch 捕获子任务异常后,
     *  CONCURRENT_EXIT 仍重抛").  By skipping settled cells we ensure
     *  an exception already caught inside the block is not re-raised
     *  by the join, while still propagating the first failure of any
     *  child that is still running when the block exits.  Still-pending
     *  children are awaited and their first failure is re-thrown
     *  (parent-wins, with sibling cancellation), exactly like [join]. */
    fun joinPendingOnly() {
        if (cancelled) {
            synchronized(childLock) { for (c in children) c.cell.cancel() }
        }
        var firstError: Throwable? = null
        for (c in children) {
            // Already settled before join: result/failure was already
            // observed (e.g. via `await`).  Don't re-await — re-awaiting
            // a FAILED cell would re-throw and defeat a user try/catch.
            if (c.cell.isDone()) continue
            if (firstError != null) {
                // A prior pending child already failed: cancel this one
                // if it hasn't settled yet, then await to collect it.
                c.cell.cancel()
            }
            try {
                c.cell.await()
