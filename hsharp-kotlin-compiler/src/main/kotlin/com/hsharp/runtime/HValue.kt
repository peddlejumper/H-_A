...
/**
 * Thread-local set of [HValue]s currently being rendered by
 * `toDisplayString()`.  Used by [HList] and [HDict] to detect circular
 * references and avoid unbounded recursion (which would otherwise throw
 * [StackOverflowError] inside `str()` / `print()`).
 *
 * Backed by an [java.util.IdentityHashMap] so membership is tested by
 * *reference* identity rather than `equals`/`hashCode`.  This is essential:
 * `HList`/`HDict` are data classes whose `hashCode()` recurses into their
 * contents, so a plain `mutableSetOf` would itself overflow the stack the
 * first time a self-referential container is `add`ed.
 */
private val displayStack = ThreadLocal.withInitial {
    java.util.Collections.newSetFromMap(java.util.IdentityHashMap<HValue, Boolean>())
}
...
data class HList(val items: MutableList<HValue>) : HValue {
    ...
    override fun toDisplayString(): String {
        if (!displayStack.get().add(this)) return "[...]"  // 循环引用
        try {
            return "[" + items.joinToString(", ") { it.toDisplayString() } + "]"
        } finally {
            displayStack.get().remove(this)
        }
    }
...
data class HDict(val entries: MutableMap<String, HValue>) : HValue {
    ...
    override fun toDisplayString(): String {
        if (!displayStack.get().add(this)) return "{...}"  // 循环引用
        try {
            return "{" + entries.entries.joinToString(", ") { (k, v) -> "${k}: ${v.toDisplayString()}" } + "}"
        } finally {
            displayStack.get().remove(this)
        }
    }
...
data class HFunction(
    ...
    // Default values for the *trailing* parameters, aligned with the
    // tail of `args`.  E.g. `fn f(a, b, c=1, d=2)` has
    // args=["a","b","c","d"] and defaultArgs=[HNumber(1), HNumber(2)].
    // At call time, missing trailing args are filled from here.  Empty
    // list means no defaults.  Currently the compiler only supports
    // literal default values (numbers, strings, bools, null).
    val defaultArgs: List<HValue> = emptyList(),
    // `fn f(...args)` or `fn f(a, b, ...rest)` — when true, the LAST
    // entry of `args` is the variadic capture name and collects all
    // trailing positional args into an HList at call time.  Mutually
    // exclusive with `defaultArgs` (enforced by the parser).
...
class HChannel(
    ...
    fun recv(): HValue {
        while (true) {
            try {
                val v = queue.poll(100, java.util.concurrent.TimeUnit.MILLISECONDS)
                if (v != null) return v  // 成功取到值，直接返回
                // poll 超时返回 null
                if (closed && queue.isEmpty()) {
                    throw HSharpException(HString("recv on closed channel"))
                }
                // 否则继续循环
    ...
    fun close() {
        closed = true
        // 不需要 drainTo + re-offer，直接标记关闭
        // queue 中已有的值保留，recv 仍可取到
    }
...
object HValueOps {
    fun equals(a: HValue, b: HValue): Boolean {
        if (a === b) {
            // NaN 不等于自身，即使是同一对象
            if (a is HNumber && a.value.isNaN()) return false
            return true
        }
        return when {
            a is HNull && b is HNull -> true
            a is HBool && b is HBool -> a.value == b.value
            a is HBool && b is HNumber -> (if (a.value) 1.0 else 0.0) == b.value
            a is HNumber && b is HBool -> a.value == (if (b.value) 1.0 else 0.0)
    ...
    fun toBool(v: HValue): Boolean = when (v) {
        is HBool -> v.value
        is HNull -> false
        else -> throw HSharpRuntimeError("expected bool, got ${v.type}")
    }
    ...
    fun toDouble(v: HValue): Double = when (v) {
        ...
        is HString -> v.value.toDoubleOrNull()
            ?: throw HSharpRuntimeError("cannot coerce STRING '${v.value}' to number")
        is HNull -> 0.0
        else -> throw HSharpRuntimeError("cannot coerce ${v.type} to number")
    }

    fun toLong(v: HValue): Long = when (v) {
        is HNumber -> v.value.toLong()
        is HBool -> if (v.value) 1L else 0L
        else -> throw HSharpRuntimeError("cannot coerce ${v.type} to int")
    }
}
