...
class HbcReader {
    ...
    private fun parseFunction(m: Map<*, *>): HFunction {
        ...
        val typeParams = (m["type_params"] as? List<*>)
            ?.map { it.toString() } ?: emptyList()
        // Default values for trailing parameters.  The compiler emits
        // them as already-typed HValue JSON values (numbers, strings,
        // bools, null) so we can parseValue them directly.
        val defaultArgs = (m["defaults"] as? List<*>)
            ?.map { parseValue(it) } ?: emptyList()
        // `fn f(...args)` — the last param collects trailing args.
        val isVariadic = (m["is_variadic"] as? Boolean) ?: false
