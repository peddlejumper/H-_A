...
> ⚠️ **历史文档标注**
> 本文撰写于 v0.4 阶段（2026-05-22），记录当时的状态。
> 当前版本为 v0.4.1（2026-06-20），主要变化：
> - 主运行时已从 Python VM 迁移到 Kotlin HVM 栈机解释器（见 `hsharp-kotlin-compiler/README.md`）
> - .hbc 格式确认为 JSON 容器（非二进制），详见 `.project_context/HBC_FORMAT_SPEC.md`
> - 文中"下一步计划"提及的泛型/async/await/模式匹配/Channel 等特性在 v0.4.1 已全部实现
> - 最新状态请参阅 `H#_v0.4.1_Package/CHANGELOG/CHANGELOG.md` 与 `.project_context/PROGRESS.md`
>
> 以下为原文内容，作为 v0.4 阶段历史记录保留。
