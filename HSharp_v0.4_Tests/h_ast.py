...
class Function(AST):
    def __init__(self, name, params, body, is_static=False, type_params=None,
                 ...
                 defaults=None, is_variadic=False):
        self.name = name
        self.params = params
        self.body = body
        self.is_static = is_static
        self.type_params = type_params or []
        # `fn f(...args)` — when True, the LAST entry of `params` is the
        # variadic capture name and collects all trailing positional args
        # into a list.  Mutually exclusive with `defaults`.
        ...
        # Default values for trailing parameters.  `defaults` is a
        # list aligned with the *trailing* end of `params`: e.g.
        # `fn f(a, b, c=1, d=2)` has params=['a','b','c','d'] and
        # defaults=[NumberLiteral(1), NumberLiteral(2)].  An empty
        # list means no defaults.  This matches Python's convention
        # so the runtime can fill missing trailing args cheaply.
