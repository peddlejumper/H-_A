...
import { X, Code2, ChevronRight, Circle } from 'lucide-react';
import SearchPanel from '@/components/SearchPanel';
import type { editor } from 'monaco-editor';

export default function EditorPanel() {
  const { files, activeFileId, openTabs, setActiveFile, closeTab, updateFile, addFile, saveCurrentToFs, markFileClean } = useFileStore();
  const { setStatusBar, toggleCommandPalette, toggleSearchPanel } = useUIStore();
  ...
  // 监听 goto-line 事件（来自大纲面板）
  useEffect(() => {
    const handleGotoLine = (e: Event) => {
      const detail = (e as CustomEvent).detail;
      if (editorInstanceRef.current && detail?.line) {
        editorInstanceRef.current.revealLineInCenter(detail.line);
        editorInstanceRef.current.setPosition({ lineNumber: detail.line, column: 1 });
        editorInstanceRef.current.focus();
      }
    };
    window.addEventListener('goto-line', handleGotoLine);
    return () => window.removeEventListener('goto-line', handleGotoLine);
  }, []);
  ...
  const handleEditorDidMount = useCallback(
    async (editor: editor.IStandaloneCodeEditor, monaco: typeof import('monaco-editor')) => {
      ...
      editor.addCommand(monaco.KeyMod.CtrlCmd | monaco.KeyCode.KeyS, () => {
        const activeFileId = useFileStore.getState().activeFileId;
        if (activeFileId) {
          const file = useFileStore.getState().files.find(f => f.id === activeFileId);
          if (file && file.source === 'fs') {
            useFileStore.getState().saveCurrentToFs().then(() => {
              useFileStore.getState().markFileClean(activeFileId);
              useUIStore.getState().addToast({ type: 'success', message: `已保存: ${file.name}` });
            }).catch(() => {
              useUIStore.getState().addToast({ type: 'error', message: '保存失败' });
            });
          } else {
            useFileStore.getState().saveToStorage();
            useFileStore.getState().markFileClean(activeFileId);
            useUIStore.getState().addToast({ type: 'success', message: '已保存' });
          }
        }
      });
      ...
      editor.addCommand(monaco.KeyMod.CtrlCmd | monaco.KeyCode.KeyN, () => {
        ...
        useFileStore.getState().addFile(name);
        useUIStore.getState().addToast({ type: 'info', message: `新建文件: ${name}` });
      });

      editor.addCommand(monaco.KeyMod.CtrlCmd | monaco.KeyCode.KeyW, () => {
        const activeFileId = useFileStore.getState().activeFileId;
        if (activeFileId) {
          useFileStore.getState().closeTab(activeFileId);
        }
      });
    ...
    return (
      <div className="flex-1 flex items-center justify-center bg-zzw-bg">
        <div className="text-center animate-fade-in">
          ...
          <p className="text-zzw-text-dim/50 text-xs mt-1">Ctrl+N 新建文件  ·  Ctrl+P 命令面板</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="flex-1 flex flex-col min-w-0 bg-zzw-bg">
      ...
      <div className="flex items-center bg-zzw-surface border-b border-zzw-border shrink-0 overflow-x-auto">
        {openTabs.map((tabId) => {
          ...
          return (
            ...
            >
              {file.dirty && (
                <Circle className="w-2 h-2 fill-current text-zzw-yellow shrink-0" />
              )}
      ...
      <div className="flex-1 relative" ref={editorRef}>
        <SearchPanel editor={editorInstanceRef.current} />
