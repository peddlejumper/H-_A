...
namespace IDE.UI
{
    public partial class LauncherWindow : Window
    {
        ...
        private HSharpVersionInfo? _currentPackageVersion;
        ...
        private async void OnLaunchZZWCode(object? sender, RoutedEventArgs e)
        {
            AppendOutput("Launching ZZW Code...");

            var zzwDist = FindZZWCodeDist();
            if (zzwDist == null)
            {
                AppendOutput("ZZW Code dist not found. Looking for zzw-code/dist/ ...");
                return;
            }
            AppendOutput($"ZZW Code dist: {zzwDist}");

            var res = await EnvironmentChecker.CheckAsync();
            var python = res.PythonExecutable ?? "python3";

            var port = GetFreePort();
            AppendOutput($"Starting HTTP server on port {port}...");
            ...
            try
            {
                ...
                {
                    FileName = python,
                    Arguments = $"-m http.server {port} --directory \"{zzwDist}\"",
                    WorkingDirectory = zzwDist,
                    UseShellExecute = true,
                ...
                AppendOutput($"Opening ZZW Code: {url}");
                Process.Start(new ProcessStartInfo
                {
                    FileName = url,
                    UseShellExecute = true
            ...
            {
                AppendOutput("ZZW Code launch failed: " + ex.Message);
            }
        }

        private static string? FindZZWCodeDist()
        {
            var dir = Directory.GetCurrentDirectory();
            for (int i = 0; i < 12; i++)
            {
                var candidate = Path.Combine(dir, "zzw-code", "dist");
            ...
            {
                var candidate = Path.Combine(pkgDir, "zzw-code", "dist");
                if (Directory.Exists(candidate) && File.Exists(Path.Combine(candidate, "index.html")))
                    return Path.GetFullPath(candidate);
            }
        ...
        private void UpdateVersionDisplay(HSharpVersionInfo? info)
        {
            _currentPackageVersion = info;
            var label = this.FindControl<TextBlock>("VersionLabel");
            if (label == null) return;

            if (info == null || !info.IsLoaded)
            {
                var lastPkg = LauncherConfig.GetLastPackage();
                if (lastPkg != null)
                {
                    label.Text = $"上次加载: {lastPkg.Name} v{lastPkg.Version}";
                    label.Foreground = Avalonia.Media.Brushes.Gray;
                }
                else
                {
                    label.Text = $"H# v{LauncherConfig.GetLauncherVersion()}";
                    label.Foreground = Avalonia.Media.Brushes.Gray;
                }
            }
            else if (info.VersionMatch)
            {
                label.Text = $"✓ {info.PackageName ?? "package"} v{info.Version}";
                label.Foreground = Avalonia.Media.Brush.Parse("#2e8b57");
            }
            else
            {
                label.Text = $"⚠ {info.PackageName ?? "package"} v{info.Version} (启动器 v{LauncherConfig.GetLauncherVersion()})";
                label.Foreground = Avalonia.Media.Brush.Parse("#cd853f");
            }
        }
        ...
        private async void OnLoadHps(object? sender, RoutedEventArgs e)
        {
            ...
            try
            {
                ...
                // ── 版本检查 ──
                var pkgVersion = HSharpVersionInfo.FromPackage(tmp, hps);
                var launcherVer = LauncherConfig.GetLauncherVersion();
                AppendOutput($"Package version: v{pkgVersion.Version}");
                AppendOutput($"Launcher version: v{launcherVer}");
                if (pkgVersion.VersionMatch)
                {
                    AppendOutput($"✓ Version match: v{pkgVersion.Version}");
                }
                else
                {
                    AppendOutput($"⚠ Version mismatch: package v{pkgVersion.Version} vs launcher v{launcherVer}");
                }
                if (!string.IsNullOrEmpty(pkgVersion.BootstrapVersion))
                {
                    AppendOutput($"  Bootstrap version: v{pkgVersion.BootstrapVersion}");
                }
                // 记录到配置
                LauncherConfig.RecordPackageLoaded(hps, pkgVersion.Version, pkgVersion.PackageName);
                // 更新 UI
                UpdateVersionDisplay(pkgVersion);
