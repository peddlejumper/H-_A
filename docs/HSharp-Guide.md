...
- 解构赋值：`let [a, b, _] = expr;`（列表/元组按位置解包，`_` 跳过槽位）
...
- 默认参数：`fn f(a, b=1, c=2)`（Python 风格，尾部对齐，仅字面量默认值）
- 变长参数：`fn f(...args)` 或 `fn f(a, b, ...rest)`（最后一个参数收集剩余实参为 list）
...
新增 native 函数 `fmt(template, args...)`，支持位置占位符 `{0}` / `{1}` / ... 的字符串格式化。

```
print(fmt("{0} + {1} = {2}", 1, 2, 3));   # 1 + 2 = 3
let s = fmt("name={0}, age={1}", "H#", 1);
```

## 解构赋值（Destructuring）

`let` 语句支持列表/元组解构，按位置将 RHS 元素绑定到左侧变量；`_` 表示跳过对应槽位。RHS 只求值一次（存入合成临时变量）。

```
let arr = [10, 20, 30];
let [a, b, _] = arr;     # a=10, b=20, 30 被跳过
let [x, y] = [1, 2];
```

## 默认参数（Default Parameters）

函数声明可为尾部参数指定字面量默认值（Python 约定：默认参数后不能再有非默认参数）。调用时缺失的尾部实参自动从默认值填充。
...
`fn f(...args)` 或 `fn f(a, b, ...rest)`：最后一个参数前加 `...`，该参数收集所有剩余位置实参为一个 list。变长参数必须是最后一个，且不能与默认参数混用（解析器强制）。
